...
var delta = parseInt(process.argv[2]);
...
var intervalID = setInterval(function () {
    ...
    if (iter == max) {
        ...
        var jsonString2A = JSON.stringify(channel2json.dataSet, null, 2);
        var jsonString3A = JSON.stringify(channel3json.dataSet, null, 2);
        // Write the strings to the files
        fs.writeFile(file1, jsonString1A, function (err) {
            if (err) {
        ...
        fs.writeFile('./data/channel1plot.json', jsonString1B, function (err) {
            if (err) {
                console.log('Error writing file', err);
            }
            else {
        ...
        fs.writeFile('./data/channel2plot.json', jsonString2B, function (err) {
            if (err) {
                console.log('Error writing file', err);
            }
            else {
        ...
        fs.writeFile('./data/channel3plot.json', jsonString3B, function (err) {
            if (err) {
                console.log('Error writing file', err);
            }
            else {
...
// CHANNEL 1 - STREAM
// Start the stream
stream.on('channels/topic1', function (tweet) {
...
// CHANNEL 2 - STREAM
// Start the stream
stream.on('channels/topic2', function (tweet) {
    // Store the data in the temporary data structure
...
// CHANNEL 3 - STREAM
// Start the stream
stream.on('channels/topic3', function (tweet) {
    // Store the data in the temporary data structure
...
function getTimestamp(length) {
    ...
    var month = date.getMonth() + 1; // This returns 0 - January, 1 - February, ... so you will need to add 1
    var day = date.getDate(); // getDate() gets the day of the month while getDay() gets the day of the week, 0 - Sunday, 1 - Monday
